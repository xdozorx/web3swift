...
public indirect enum EIP712_ABIValue: Equatable {
    ...
    /// A function call
    case function(EIP712_Function, [EIP712_ABIValue])
    
    /// Fixed-length array where all values have the same type
    case array(EIP712_ABIType, [EIP712_ABIValue])
    ...
    case dynamicArray(EIP712_ABIType, [EIP712_ABIValue])
    
    /// Tuple
    case tuple([EIP712_ABIValue])
    
    /// Value type
    public var type: EIP712_ABIType {
        switch self {
    ...
    public init(_ value: Any, type: EIP712_ABIType) throws {
        switch (type, value) {
        case (.uint(let bits), let value as Int64):
            self = .uint(bits: bits, BigUInt(value))
        case (.uint(let bits), let value as UInt64):
            self = .uint(bits: bits, BigUInt(value))
        case (.uint(let bits), let value as BigUInt):
            self = .uint(bits: bits, value)
        case (.int(let bits), let value as Int64):
            self = .int(bits: bits, BigInt(value))
        case (.int(let bits), let value as BigInt):
            self = .int(bits: bits, value)
        case (.address, let address as String):
        ...
        case (.array(let type, _), let array as [Any]):
            self = .array(type, try array.map({ try EIP712_ABIValue($0, type: type) }))
        ...
        case (.dynamicArray(let type), let array as [Any]):
            self = .dynamicArray(type, try array.map({ try EIP712_ABIValue($0, type: type) }))
        case (.tuple(let types), let array as [Any]):
            self = .tuple(try zip(types, array).map({ try EIP712_ABIValue($1, type: $0) }))
        default:
