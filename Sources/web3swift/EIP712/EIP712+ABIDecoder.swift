...
public final class EIP712_ABIDecoder {
    let data: Data
    var offset = 0
    ...
    /// Decodes an `ABIValue`
    public func decode(type: EIP712_ABIType) throws -> EIP712_ABIValue {
        switch type {
        case .uint(let bits):
    ...
    public func decodeArray(type: EIP712_ABIType) throws -> [EIP712_ABIValue] {
        let count = Int(decodeUInt())
        return try decodeArray(type: type, count: count)
    }

    /// Decodes a static array
    public func decodeArray(type: EIP712_ABIType, count: Int) throws -> [EIP712_ABIValue] {
        return try decodeTuple(types: Array(repeating: type, count: count))
    }

    /// Decodes a tuple
    public func decodeTuple(types: [EIP712_ABIType]) throws -> [EIP712_ABIValue] {
        let baseOffset = offset

        var values = [EIP712_ABIValue]()
        for subtype in types {
            let value: EIP712_ABIValue
    ...
    public func decode(function: EIP712_Function) throws -> EIP712_ABIValue {
        if try EIP712_ABIEncoder.encode(signature: function.description) != decodeSignature() {
            throw EIP712_ABIError.functionSignatureMismatch
        }
    ...
    public func decodeUInt() -> BigUInt {
        assert(offset + EIP712_ABIEncoder.encodedIntSize <= data.count)
        let value = BigUInt(data.subdata(in: offset ..< offset + EIP712_ABIEncoder.encodedIntSize))
        offset += EIP712_ABIEncoder.encodedIntSize
        return value
    }
    ...
    public func decodeInt() -> BigInt {
        ...
        let max = BigInt(1) << (EIP712_ABIEncoder.encodedIntSize * 8)
    ...
    public func decodeAddress() -> String {
        ...
        let ethereumAddress = data.subdata(in: offset + EIP712_ABIEncoder.encodedIntSize - addressLength ..< offset + EIP712_ABIEncoder.encodedIntSize).toHexString()
        offset += EIP712_ABIEncoder.encodedIntSize
    ...
    public func decodeString() throws -> String {
        ...
        guard let string = String(data: data.subdata(in: offset ..< offset + count), encoding: .utf8) else {
            throw EIP712_ABIError.invalidUTF8String
        }
        offset += count
        return string
    }
