...
/// or strings.
public enum EIP712_JSON: Equatable {
    case string(String)
    case number(UInt64)
    case object([String: EIP712_JSON])
    case array([EIP712_JSON])
    case bool(Bool)
    case null
}

extension EIP712_JSON: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let object = try? container.decode([String: EIP712_JSON].self) {
            self = .object(object)
        } else if let array = try? container.decode([EIP712_JSON].self) {
            self = .array(array)
...
extension EIP712_JSON: CustomDebugStringConvertible {
...
public extension EIP712_JSON {
    ...
    var objectValue: [String: EIP712_JSON]? {
    ...
    subscript(index: Int) -> EIP712_JSON? {
    ...
    subscript(key: String) -> EIP712_JSON? {
